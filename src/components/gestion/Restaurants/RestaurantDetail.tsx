...
export default function RestaurantDetail({
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Impossible de charger les détails du restaurant";
  ...
  const fetchRestaurantStats = async (restaurantId: string) => {
    ...
    try {
      ...
      if (
        revenue === "0 XOF" &&
        apiData.dailySales &&
        apiData.dailySales.subtitle
      ) {
        const dailySalesMatch =
          apiData.dailySales.subtitle.match(/(\d[\d\s]*)/);
        if (dailySalesMatch) {
          const revenueNumeric =
            parseInt(dailySalesMatch[1].replace(/\s/g, "")) || 0;
          if (revenueNumeric > 0) {
      ...
      console.error(
        "❌ Erreur lors du chargement des statistiques du restaurant:",
        error
      );
  ...
  const fetchRestaurantMenus = useCallback(async (restaurantId: string) => {
    ...
    try {
      ...
      const menusByRestaurantId = allMenus.filter(
        (menu) => menu.restaurantId === restaurantId
      );

      // Méthode 2: Vérifier si le restaurantId est un tableau et contient l'ID recherché
      const menusByRestaurantIdArray = allMenus.filter(
        (menu) =>
          Array.isArray(menu.restaurantId) &&
      ...
      if (filteredMenus.length > 0) {
        toast.success(
          `${filteredMenus.length} menus trouvés pour ce restaurant`
        );
      } else {
      ...
      console.error(
        "Erreur lors du chargement des menus du restaurant:",
        error
      );
      const errorMessage =
        error instanceof Error
  ...
  const handleToggleActive = async (active: boolean) => {
    ...
    try {
      ...
      toast.success(
        `Restaurant ${active ? "activé" : "désactivé"} avec succès`
      );
    } catch (error: unknown) {
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Impossible de mettre à jour le statut du restaurant";
      toast.error(`Erreur: ${errorMessage}`);
    ...
    return sortedSchedule.map((item) => {
      ...
      return (
        ...
        >
          ...
          <span
            className={`text-sm ${
              isClosed ? "text-red-500 italic" : "text-green-600"
            } font-medium`}
          >
                ...
                <div className="relative w-full h-48">
                  {restaurant.image && restaurant.image.trim() !== "" ? (
                    <Image
                      ...
                      onError={(e) => {
                        console.error(
                          "Erreur de chargement image restaurant:",
                          restaurant.image
                        );
                ...
                {isLoadingStats ? (
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div className="bg-gray-50 p-3 rounded-xl text-center">
                      ...
                      <p className="text-xl font-bold text-[#F17922]">
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-5">
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                      ...
                      .map((menu) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div className="w-12 h-12 border-[1px] border-orange-100 rounded-md flex items-center justify-center overflow-hidden">
                              {menu.image && menu.image.trim() !== "" ? (
                                <Image
                                  ...
                                  onError={(e) => {
                                    console.error(
                                      "Erreur de chargement image menu:",
                                      menu.image
                                    );
                                    e.currentTarget.style.display = "none";
                                  }}
                            ...
                            <div>
                              <p className="text-sm font-medium text-[#F17922]">
                                {menu.name}
                              </p>
                              <div className="flex items-center gap-2">
                                <p className="text-xs text-gray-500">
                                  {(menu.isPromotion || menu.is_promotion) &&
                                  menu.originalPrice &&
                                  menu.discountedPrice ? (
                                    <span className="flex items-center">
                                      <span className="line-through mr-1">
                                        {
                                          (menu as { originalPrice?: string })
                                            .originalPrice
                                        }{" "}
                                        XOF
                                      </span>
                                      <span className="text-green-600 font-medium">
                                        {
                                          (menu as { discountedPrice?: string })
                                            .discountedPrice
                                        }{" "}
                                        XOF
                                      </span>
                  ...
                  <button
                    ...
                    onClick={() =>
                      restaurant?.id && fetchRestaurantMenus(restaurant.id)
                    }
                  >
                    {isLoadingMenus ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="mt-1">
                      {(selectedMenu.isPromotion ||
                        selectedMenu.is_promotion) &&
                      selectedMenu.originalPrice &&
                      selectedMenu.discountedPrice ? (
                        <div className="flex items-center gap-2">
                          <span className="line-through text-gray-400">
                            {
                              (selectedMenu as { originalPrice?: string })
                                .originalPrice
                            }{" "}
                            XOF
                          </span>
                          <span className="text-green-600 font-medium">
                            {
                              (selectedMenu as { discountedPrice?: string })
                                .discountedPrice
                            }{" "}
                            XOF
                          </span>
                        ...
                        <p className="text-gray-600">
                          {selectedMenu.price} XOF
                        </p>
                      )}
                    </div>
                  </div>

                  {(selectedMenu as { ingredients?: string[] }).ingredients &&
                    (selectedMenu as { ingredients?: string[] }).ingredients!
                      .length > 0 && (
                      <div>
                        <h4 className="text-sm font-medium text-gray-700">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {(
                            selectedMenu as { ingredients?: string[] }
                          ).ingredients!.map(
                  ...
                  {selectedMenu.supplements &&
                    Object.keys(selectedMenu.supplements).length > 0 && (
                      <div>
                        <h4 className="text-sm font-medium text-gray-700">
                          Suppléments disponibles
                        </h4>
                        <div className="mt-1 space-y-2">
                          {Object.entries(selectedMenu.supplements).map(
                            ([key, value]: [
                              string,
                              {
                                items?: Array<{
