...
import ClickActionPicker, { buildClickData, restoreClickAction } from "../ClickActionPicker";
...
export default function CreateScheduledModal({
  ...
  useEffect(() => {
    if (editItem) {
      ...
      const restored = restoreClickAction(editItem.payload?.data as Record<string, unknown> | undefined);
      setClickAction(restored.action);
      setClickValue(restored.value);
