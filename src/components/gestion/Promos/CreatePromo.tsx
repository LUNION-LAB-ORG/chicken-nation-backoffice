...
import { ApiPromotion, PromoTransitData } from "@/services/promotionService";
...
const CreatePromo = ({
  ...
  const [activePromoType, setActivePromoType] =
    useState<PromoType>("percentage");
  const [activeProductTarget, setActiveProductTarget] =
    useState<ProductTarget>("all");
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const menuData = await getAllMenus({ page: 1, limit: 100 });
        setMenus(
          menuData.filter((m) => m.isAvailable) as unknown as MenuItem[]
        ); // Seulement les menus disponibles
  ...
  const handleSelectAllRestaurants = () => {
    if (selectedRestaurants.length === restaurants.length) {
      ...
      setSelectedRestaurants(
        restaurants.map((r) => r.id || "").filter(Boolean)
      );
    }
  };
  ...
  const handleSelectAllPublicTypes = () => {
    ...
    const privateTypesSelected = publicTypeOptions.filter((type) =>
      selectedPublicTypes.includes(type)
    );
    ...
    if (privateTypesSelected.length === publicTypeOptions.length) {
      ...
      setSelectedPublicTypes((prev) =>
        prev.filter((type) => !publicTypeOptions.includes(type))
      );
    } else {
  ...
  const getSelectedItemsDisplay = (
    ...
    if (selectedIds.length <= maxDisplay) {
      ...
      const displayItems = selectedItems
        ...
        .map((item) => item.name)
        .join(", ");
      return `${displayItems} +${selectedIds.length - maxDisplay} autre${
        selectedIds.length - maxDisplay > 1 ? "s" : ""
      }`;
    }
  };
  ...
  const validateCreatePromoStep = () => {
    ...
    if (activeProductTarget === "specific" && selectedMenus.length === 0) {
      errors.push(
        "Vous devez sélectionner au moins un produit pour une promotion ciblée"
      );
    }
    ...
    if (
      ...
      errors.push(
        "Vous devez sélectionner au moins une catégorie pour une promotion ciblée"
      );
    }
  ...
  useEffect(() => {
    if (initialData && isEditing) {
      ...
      if (isApiPromotion) {
        ...
        setActivePromoType(
          apiPromotion.discount_type === "PERCENTAGE"
            ? "percentage"
            : apiPromotion.discount_type === "FIXED_AMOUNT"
            ? "fixed"
            : "buyXgetY"
        );

        setActiveProductTarget(
          apiPromotion.target_type === "ALL_PRODUCTS"
            ? "all"
            : apiPromotion.target_type === "SPECIFIC_PRODUCTS"
            ? "specific"
            : "categories"
        );
        ...
        setActivePromoType(
          promoCard.type === "percentage"
            ? "percentage"
            : promoCard.type === "fixed"
            ? "fixed"
            : "buyXgetY"
        );
