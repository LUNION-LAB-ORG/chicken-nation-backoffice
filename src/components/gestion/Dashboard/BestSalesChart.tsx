...
import { BestSalesItem } from "../../../../features/statistics/types/statistics.types";
import { getBestSalesData } from "../../../../features/statistics/services/statistics.service";
...
const MiniBarChart: React.FC<{ count: number }> = ({ count }) => {
  return (
    <div className="flex gap-2 flex-row items-center justify-center">
      ...
      <span className="text-[16px] text-[#9796A1] font-medium mt-1">
        {count}
      </span>
    </div>
  );
...
const BestSalesChart: React.FC<BestSalesChartProps> = ({
  title,
  items,
  restaurantId,
  period = "month",
}) => {
  const [bestSalesData, setBestSalesData] = useState<BestSalesItem[]>(
    items || []
  );
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          {bestSalesData.map((item) => (
            <div
              key={item.id}
              className="flex items-center justify-between py-2  border-b-[1px] border-[#E6E6E6]  "
            >
              <div className="flex items-center mx-2">
                ...
                <div>
                  <h4 className="lg:text-[13px] text-[10px] font-bold text-[#595959] lg:line-clamp-2 line-clamp-3 lg:w-60 w-26 mr-4">
                    {item.name}
                  </h4>
                  <div className="flex items-center gap-1 mt-1">
                    <Image
                      className="mt-1 contain-content lg:w-4 lg:h-4 w-3 h-3"
                      src="/icons/stonk.png"
                      alt="circle"
                      width={14}
                      height={14}
                    />
                    <span className="lg:text-xs text-[10px] line-clamp-2 lg:line-clamp-1 lg:w-50 w-20 text-[#F17922]">
                      {item.interestedPercentage}
                    </span>
                  </div>
                </div>
              </div>
