...
const SelectWithCheckboxes = ({
  ...
  useEffect(() => {
    ...
    if (validValues.length !== value.length) {
      const hasRealDifference =
        value.some((v) => !validValues.includes(v)) ||
  ...
  const renderDropdown = () => {
    ...
    return createPortal(
      ...
      >
        <div
          className="absolute z-50 bg-white border border-white  rounded-b-xl shadow-lg overflow-hidden"
          style={{
    ...
    <div className="relative" ref={containerRef}>
      ...
      >
        <div className="flex items-center justify-center gap-2">
          <span className="text-[#595959] font-semibold text-[13px]">
            Sélectionnez les restaurants
          </span>
          <Image
            src="/icons/down.png"
            alt="arrow"
            width={4}
            height={4}
            className={`w-1 xs:w-3 xs:h-2 sm:w-4 sm:h-2 text-gray-400 transition-transform duration-200 flex-shrink-0 ${
              isOpen ? "rotate-180" : ""
            }`}
          />
        </div>
