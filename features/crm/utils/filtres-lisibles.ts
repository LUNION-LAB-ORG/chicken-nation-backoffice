...
import { ETAT_COUPON_META, PUBLIC_META, STATUT_META } from "./crm-ui";
...
function valeurLisible(cle: string, valeur: string): string {
  if (cle === "status") {
    return valeur
      .split(",")
      .map((s) => STATUT_META[s as ContactStatut]?.label ?? s)
      .join(", ");
  }
  if (cle === "segment") return PUBLIC_META[valeur as Public]?.label ?? valeur;
