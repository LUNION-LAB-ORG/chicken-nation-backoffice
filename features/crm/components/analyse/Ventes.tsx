...
import { PUBLIC_META, accord, fmtDate, fmtMontant, fmtNombre, fmtPct } from "../../utils/crm-ui";
...
function ParMois({ v }: { v: IVentes }) {
  ...
  return (
    <Tableau colonnes={["Mois", "Public", "Ventes", "Chiffre d'affaires"]}>
      {v.par_mois.map((m) => (
          ...
          <Cellule gauche>
            ...
            {m.historique > 0 && (
              <span className="block text-xs text-gray-400">
                {m.historique === m.ventes ? "historique acquisition" : `dont ${fmtNombre(m.historique)} en historique acquisition`}
              </span>
            )}
          </Cellule>
          <Cellule gauche>
...
function CapturesParRestaurant({ v }: { v: IVentes }) {
  ...
  return (
    <Tableau colonnes={["Restaurant", "Commandes relevées", "Clients", "Ont commandé en direct", "Taux"]}>
 ...
 * puces). Remplace les ventes de l'ancienne acquisition Glovo/Yango, dont les
 * mois d'avant la bascule restent visibles en « historique acquisition ».
 */
export function Ventes({ onOuvrir }: { onOuvrir: (id: string) => void }) {
  ...
  const requete = useVentesQuery({ from: periode.from, to: periode.to, segment, restaurant_id: restaurantId || undefined });
    ...
    <div className="space-y-4">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <ChoixPublic valeur={segment} onChange={setSegment} />
          <div className="w-64">
            <ChampSelect
              valeur={restaurantId}
      ...
      {requete.isError ? (
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
            <StatsCard
              title="Ventes"
              value={fmtNombre(v.total.ventes)}
              subtitle={v.total.historique > 0 ? `dont ${fmtNombre(v.total.historique)} en historique acquisition` : "clients sortis du CRM par une commande"}
              color="green"
            />
            <StatsCard title="Chiffre d'affaires" value={fmtMontant(v.total.ca)} color="green" />
            <StatsCard title="Panier moyen" value={fmtMontant(v.total.panier_moyen)} />
            <StatsCard
              title="Clients Glovo/Yango relevés"
              value={fmtNombre(v.captures_par_restaurant.reduce((s, r) => s + r.personnes, 0))}
              subtitle={`${fmtNombre(v.captures_par_restaurant.reduce((s, r) => s + r.ventes, 0))} ${accord(
                v.captures_par_restaurant.reduce((s, r) => s + r.ventes, 0),
                "a commandé",
                "ont commandé",
              )} en direct`}
              color="blue"
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
              {v.par_public.map((p) => (
                <div key={p.segment} className="bg-white border border-gray-200 rounded-xl px-4 py-3">
                  ...
                  <p className="text-xl font-bold text-gray-900 tabular-nums mt-2">
                    {fmtNombre(p.ventes)} {accord(p.ventes, "vente")}
                  </p>
          ...
          <p className="flex items-start gap-2 text-xs text-gray-500">
            ...
            <span>
              ...
              {v.bascule && ` Les ventes antérieures au ${fmtDate(v.bascule)} viennent de l'ancienne acquisition Glovo/Yango.`}
            </span>
          </p>
        </>
      )}
