...
const LoyaltyFilterTab: React.FC = () => {
  ...
  const handleSearchChange = (value: string) => {
    setFilter("loyalty", "search", value);
    setPagination("loyalty", 1, 10);
  };
  ...
  return (
    <div className="mb-6 w-full space-y-4">
      {/* Recherche */}
      <div className="flex items-center w-full bg-white rounded-lg border border-gray-200">
      ...
      <div className="w-full overflow-x-auto">
        ...
        >
          <button
            className={`transition-colors font-bold cursor-pointer text-[11px] lg:text-[14px] px-3 sm:px-5 py-1 rounded-[12px] focus:outline-none whitespace-nowrap flex-shrink-0
                ${
          ...
          {pointTypes.map((type) => (
            <button
              key={type.key}
              className={`transition-colors font-bold cursor-pointer text-[11px] lg:text-[14px] px-3 sm:px-5 py-1 rounded-[12px] focus:outline-none whitespace-nowrap flex-shrink-0 ml-1
                ${
