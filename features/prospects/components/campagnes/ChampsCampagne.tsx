...
export function ChampsCampagne({ f, maj, lancee }: { f: EtatForm; maj: (p: Partial<EtatForm>) => void; lancee: boolean }) {
  ...
  return (
    <div className="space-y-4">
      <ChampTexte label="Nom" requis valeur={f.name} onChange={(v) => maj({ name: v })} placeholder="Relance prospects octobre 2026" />
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        ...
        <ChampTexte label="Prospects à traiter" type="number" valeur={f.target_contacts_count} onChange={(v) => maj({ target_contacts_count: v })} min={1} />
      </div>
      ...
      <fieldset disabled={lancee} className="space-y-3 disabled:opacity-60">
        <div className="grid sm:grid-cols-2 gap-3">
          <ChampSelect
            ...
            options={[{ value: "tous", label: "Tous les prospects actifs au lancement" }, { value: "periode", label: "Inscrits sur une période" }]}
          />
          <ChampSelect
            label="Répartition"
