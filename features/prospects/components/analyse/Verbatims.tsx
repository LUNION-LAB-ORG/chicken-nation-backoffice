...
import { fmtDateHeure, fmtNombre } from "../../utils/prospect-ui";
...
export function Verbatims({ periode, onOuvrir }: { periode: IPeriode; onOuvrir: (id: string) => void }) {
  ...
  const requete = useVerbatimsQuery({ ...periode, search: recherche || undefined, page });
  const { data } = requete;
  ...
  return (
    <StatsChartCard title="Verbatims" subtitle={data ? `${fmtNombre(total)} commentaire${total > 1 ? "s" : ""} d'agents` : "Commentaires d'agents"} icon={MessagesSquare}>
