...
export function KpisGlobaux({ v }: { v: IVueEnsemble }) {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      <StatsCard title="Prospects actifs" value={fmtNombre(p.actifs)} subtitle={`${fmtNombre(p.jamais_appeles)} ${accord(p.jamais_appeles, "jamais appelé")}`} />
      ...
      <StatsCard
        ...
        subtitle={
          commandes > 0
            ? `${String(c.delai_moyen_jours).replace(".", ",")} j en moyenne après l'inscription`
            : "aucun inscrit de la période n'a encore commandé"
        }
        color="green"
      />
      <StatsCard
