...
const TEINTES = ["#FDBA74", "#FB923C", "#F97316", "#EA580C", "#C2410C", "#16A34A"];
...
export function Entonnoir({ etapes }: { etapes: IVueEnsemble["entonnoir"] }) {
  ...
  return (
    <StatsChartCard title="Entonnoir de conversion" subtitle="Prospects inscrits sur la période, étape par étape" icon={Filter}>
      <ol className="space-y-2">
        {etapes.map((e, i) => (
          <li key={e.cle} className="grid grid-cols-[110px_1fr_auto] items-center gap-3 text-sm">
            ...
            <div className="h-7 bg-gray-50 rounded-lg overflow-hidden">
              <div
                ...
                style={{ width: `${Math.max(4, (e.nombre / base) * 100)}%`, backgroundColor: TEINTES[i] }}
              >
                {fmtNombre(e.nombre)}
              </div>
            </div>
