...
import { depuis, fmtDate, fmtDateHeure, fmtTelephone } from "../../utils/prospect-ui";
import { PuceCoupon, PuceStatut } from "../commun/Puces";

/** Une ligne du tableau (bureau). Le clic ouvre la fiche, la case ne l'ouvre pas. */
export function LigneProspect({
  p,
  coche,
  onCocher,
  onOuvrir,
  ...
  return (
    <tr onClick={onOuvrir} className={`border-t border-gray-100 hover:bg-gray-50 cursor-pointer ${coche ? "bg-orange-50/60" : ""}`}>
      ...
      <td className="px-4 py-3 whitespace-nowrap">
        <p className="text-gray-700 tabular-nums">{fmtDate(p.registered_at)}</p>
        <p className="text-xs text-gray-400">{depuis(p.registered_at)}</p>
      </td>
      <td className="px-4 py-3">
