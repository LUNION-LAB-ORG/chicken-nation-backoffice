...
export function FiltresAvances({
  ...
  onChange: (partiel: Partial<IProspectFiltres>) => void;
}) {
  ...
  return (
    <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 border-t border-gray-100 pt-3">
      <ChampSelect
        ...
        onChange={(v) => onChange({ coupon: (v || undefined) as IProspectFiltres["coupon"] })}
        vide="Peu importe"
        options={[
      ...
      <ChampSelect
        label="Trier"
        valeur={filtres.sort ?? "inscription_desc"}
        onChange={(v) => onChange({ sort: v as IProspectFiltres["sort"] })}
        options={TRIS}
      />
