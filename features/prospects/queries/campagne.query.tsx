...
export const useCampagnesQuery = () =>
  useQuery({ queryKey: conversionKeyQuery("campagnes"), queryFn: () => campagneAPI.obtenirTous(), staleTime: 30_000 });

export const useCampagneStatsQuery = (id: string | null) =>
  useQuery({
    queryKey: conversionKeyQuery("campagnes", "stats", id),
...
export const useComparatifQuery = (actif: boolean) =>
  useQuery({ queryKey: conversionKeyQuery("campagnes", "comparatif"), queryFn: campagneAPI.comparer, enabled: actif });
  ...
  return useMutation({
    mutationFn: async ({ id, geste }: { id: string; geste: Geste }) => {
      if (geste === "lancer") {
        ...
        return `Campagne lancée : ${r.cibles} prospects, ${r.repartis} répartis`;
      }
      if (geste === "terminer") {
        const r = await campagneAPI.terminer(id);
        return `Campagne terminée : ${r.liberes} prospects libérés, rapport figé`;
      }
...
export const useDistribuerMutation = () => {
  ...
  return useMutation({
    ...
    onSuccess: (r) => {
      invalider();
      toast.success(r.repartis ? `${r.repartis} prospects répartis` : "Rien à répartir : tout le monde a son agent");
    },
    onError: (e: Error) => toast.error(e.message),
  });
};
