...
export function ClientDetail({ clientId }: ClientProfileProps) {
  const {
  ...
  useEffect(() => {
    const loadClientData = async () => {
      ...
      try {
        ...
        try {
          const reviewsData = (await getCustomerReviews(
            clientId
    ...
    return (
      <div className="bg-white p-4 sm:p-6 lg:px-8 xl:px-24 2xl:px-64 rounded-xl shadow-sm text-center py-8">
        ...
        <p className="text-gray-500 mb-6">
          Ce client n&apos;existe plus, a été supprimé ou vous n&apos;avez pas
          les permissions pour le consulter.
        </p>
        <button
          type="button"
          onClick={() => setSectionView("clients", "list")}
  ...
  const orderHistory = clientOrders.map((order) => ({
    ...
    paiements: Array.isArray(order.paiements)
      ? order.paiements.map((p: unknown) => ({
    ...
    <>
      <div className="bg-white p-4 sm:p-6 lg:px-8 xl:px-24 2xl:px-64 rounded-xl shadow-sm">
        <div className="flex flex-col items-center">
          ...
          <h2 className="text-base sm:text-lg mt-4 font-medium font-pro text-gray-900">
            {fullName}
          </h2>
        </div>
        ...
        <div className="mt-6 w-full max-w-[1400px] mx-auto">
          {activeTab === "identification" && (
            <IdentificationTab
              ...
              fullName={fullName}
              formatDate={dateToLocalString}
            />
          )}

          {activeTab === "commandes" && (
            <OrdersTab client={adaptedClient} formatDate={dateToLocalString} />
          )}
