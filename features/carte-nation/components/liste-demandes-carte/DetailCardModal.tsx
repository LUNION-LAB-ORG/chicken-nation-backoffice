...
export function DetailCardModal({ request, onClose }: DetailCardModalProps) {
  ...
  // Approuver et rejeter : PATCH .../requests/:id/review (CARD_NATION UPDATE).
  // Supprimer : DELETE .../requests/:id (CARD_NATION DELETE).
  ...
  const peutTrancher = isPending && peutValider;
  const aDesActions = peutTrancher || peutSupprimer;
      ...
      >
        ...
        {request.revision_reason && (
          <div className="mx-6 mb-6 rounded-xl border border-amber-200 bg-amber-50 p-3">
            ...
            {/* Consigne d'approbation : inutile à qui ne peut pas approuver. */}
            {peutTrancher && (
