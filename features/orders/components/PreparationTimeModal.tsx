...
const PreparationTimeModal: React.FC<PreparationTimeModalProps> = ({
  ...
  const [selectedPreparationTime, setSelectedPreparationTime] = useState<
    number | null
  >(null);
  const [customPreparationTime, setCustomPreparationTime] =
    useState<number>(15);
  ...
  const handleConfirm = async () => {
    if (selectedPreparationTime && orderId) {
      try {
        ...
        console.error(
          "❌ Erreur lors de la sauvegarde du temps de préparation:",
          error
        );
        ...
        {orderLoading ? (
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {orderData.order_items.map((item, index) => (
                  <div
                    key={item.id || index}
                    className="flex items-center space-x-3 r p-3  "
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {item.epice ? (
                        ...
                        <div>
                          {" "}
                          <span className="bg-green-100 text-slate-600 px-2 py-0.5 rounded-full text-xs">
                            🌿Non Épicé
                          </span>
                        </div>
                      )}
          ...
          <div className="mb-8 p-4 bg-yellow-50 rounded-xl border border-yellow-200">
            <div className="text-center text-yellow-800">
              ...
              <p className="text-sm mt-1">
                {!orderData
                  ? "Données de commande non chargées"
                  : !orderData.order_items
                  ? "Pas d'items dans la commande"
            ...
            {predefinedTimes.map((time, index) => (
              <div
                ...
                className={`p-4 rounded-2xl border-2 cursor-pointer transition-all hover:shadow-md ${
                  selectedPreparationTime === time.prep && !useCustom
            ...
            <div className="relative">
              ...
              <div className="relative h-6 bg-white rounded-full overflow-hidden">
                ...
                <div
                  ...
                  style={{
                    width: `${
                      ((customPreparationTime - 5) / (200 - 5)) * 100
                    }%`,
                  }}
                ></div>
                ...
                <div
                  ...
                  style={{
                    left: `${((customPreparationTime - 5) / (200 - 5)) * 100}%`,
                  }}
                >
                  <div
                    className={`w-8 h-8 rounded-full border-4 border-white shadow-lg transition-all duration-200 ${
                      useCustom
                  ...
                  {/* Tooltip flottant */}
                  <div
                    className={`absolute -top-12 left-1/2 transform -translate-x-1/2 px-3 py-1 rounded-lg text-xs font-medium transition-all duration-200 ${
                      useCustom
                  ...
                  >
                    ...
                    {/* Flèche */}
                    <div
                      className={`absolute top-full left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[4px] border-transparent ${
